...
if( ! function_exists('bravad_block_animation')){
	/**
	 * Get block animation
	 */
	function bravad_block_animation(){
		$animation = get_sub_field( 'animation' );

		if ( ! empty( $animation ) ) {
			return sprintf( ' data-aos="%s"', $animation );
		}
	}
}
...
if ( ! function_exists( 'bravad_layout' ) ) {
		...
		function bravad_layout( $layout ) {
			$names = array(
				...
				'descriptions-de-poste' => 'jobs',
				'texte-colonnes'    => 'text-columns',
			);
